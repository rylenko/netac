...
type CopiesImpl struct {
	inner []Copy
	mutex sync.Mutex
}

func (copies *CopiesImpl) DeleteExpired(ttl time.Duration) {
	copies.mutex.Lock()
...
func (copies *CopiesImpl) Print(dest io.Writer) error {
	copies.mutex.Lock()
	defer copies.mutex.Unlock()
...
func (copies *CopiesImpl) Register(newCopy Copy) {
	copies.mutex.Lock()
	defer copies.mutex.Unlock()

	for _, copy := range copies.inner {
		if copy.Equal(newCopy) {
			copy.ProlongUntil(newCopy)
			return
		}
	}
